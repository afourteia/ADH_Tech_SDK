...
enum
{
	    ...
	    NACK_RAM_ERROR,						
        NACK_TEMPLATE_CAPACITY_FULL	,
        NACK_COMMAND_NO_SUPPORT	,    
};
enum
{
		OEM_NONE					= -2000,
...
typedef struct _ST_MODULE_INFO_
{
	unsigned char		m_szSensor[12];
	unsigned char		m_szEngineVer[12];
	unsigned short		m_nRawImgWidth;
	unsigned short		m_nRawImgHeight;
	unsigned short		m_nImgWidth;
	unsigned short		m_nImgHeight;
	unsigned short		m_nMaxRecordCnt;
	unsigned short		m_nEnrollCnt;
	unsigned short		m_nTemplateSize;
...
#define FP_TEMPLATE_SIZE_MAX	5000

#define GT_ENROLL_COUNT		3
#define GH_ENROLL_COUNT		10
#define FINGER_TEMPLATE_SIZE	498


extern BYTE	gbyImg8bit[IMAGE_SIZE_MAX];
extern BYTE	gbyImgRaw[IMAGE_SIZE_MAX];
extern BYTE	gbyTemplate[FP_TEMPLATE_SIZE_MAX];
...
extern ST_MODULE_INFO s_MouldeInfo;
